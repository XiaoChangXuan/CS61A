...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            # Check if the existing ant can contain the new ant
            if place.ant.can_contain(self):
                place.ant.store_ant(self)
            # Check if the new ant can contain the existing ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur_index = 0
        while not cur_place.is_hive: # don't arrive hive
            if cur_place.bees and self.min_range <= cur_index <= self.max_range:
...
# The WallAnt class
class WallAnt(Ant):
    """"WallAnt has many health , but no action"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """Hungry Ant can eat bee, then it need relase 3 times"""

    name = 'Hungry'
    damage = 10000 # enough hight
    food_cost = 4
    time_to_chew = 3
    implemented = True
    ...
    def action(self, gamestate):
        if self.chew_timer != 0:  # chew_timer not equal zero
            self.chew_timer -= 1
        else:
            if self.place.bees:
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2):
        """
        Create a BodyguardAnt with an initial health of 2.
        """
        super().__init__(health)
    # END Problem 8
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is waterproof insects, and have same attribution of ThrowerAnt"""
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        # Check if a QueenAnt has already been constructed
        if cls.constructed:
            return None
        # Call superclass construct method
        queen = super().construct(gamestate)
        cls.constructed = True  # Mark QueenAnt as constructed
        return queen
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        def double_damage(ant):
            """Double the damage of the given ant if it can take damage."""
            if ant and not ant.is_doubled_damage:
                ant.damage *= 2
                ant.is_doubled_damage = True  # Mark the ant as doubled damage

        def process_tunnel(place):
            """Process the ants in a tunnel, doubling their damage."""
            current_place = place.exit
            while current_place:
                current_ant = current_place.ant
                if current_ant:
                    double_damage(current_ant)
                    if current_ant.is_container and current_ant.ant_contained:
                        double_damage(current_ant.ant_contained)
                current_place = current_place.exit
            
        # Process the tunnel where the QueenAnt is located
        tunnel_place = self.place
        process_tunnel(tunnel_place)

        # Process the remaining tunnels
        while tunnel_place.exit:
            tunnel_place = tunnel_place.exit
            process_tunnel(tunnel_place)

    def remove_from(self, place):
        """
        Try to remove this ant from the given place. If it cannot be removed,
        do not remove it.
        """
        pass
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.scare_rounds:
                if not entrance.is_hive:
                    if gamestate.time % 2 == 0:  
                        self.move_to(entrance)  
                    else:
                        self.scare_rounds += 1
            elif self.slow_rounds :
                if gamestate.time % 2 == 0:
                    self.move_to(destination)
            else:
                self.move_to(destination)

            # Decrement scare and slow rounds     
            if self.scare_rounds:
                self.scare_rounds -= 1
            if self.slow_rounds:
                self.slow_rounds -= 1
