...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        result = hefty_hogs(player_score,opponent_score)
    else:
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        cur_leader = 1
        diff_score = score1 - score0
    else:
        cur_leader = None
        message = None

    if cur_leader == last_leader or cur_leader == None: #leader no change or score is same
        message = None
    else:
        message = f"Player {cur_leader} takes the lead by {diff_score}"
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 10
    result_num = 0
    roll0_score = hefty_hogs(score, opponent_score)
    if roll0_score < threshold:
        result_num = num_rolls
    return result_num
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    result_num = 0
    roll0_score = hefty_hogs(score, opponent_score)
    cur_score = score + roll0_score
    if hog_pile(cur_score, opponent_score):
        result_num = 0
    elif roll0_score < threshold:
        result_num = num_rolls
    return result_num
    # END PROBLEM 11
